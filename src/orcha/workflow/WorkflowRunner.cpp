...
web::json::value WorkflowRunner::resolve_placeholders(const web::json::value& input, const std::vector<WorkflowStepResult>& previous_results) {
    // Only handles string fields with {{stepN.output.*}} placeholders
    if (input.is_string()) {
        ...
        while (std::regex_search(s, match, re)) {
            int idx = std::stoi(match[1].str()) - 1; // stepN is 1-based
...
bool WorkflowRunner::run(const std::string& yaml_path, std::vector<WorkflowStepResult>& step_results) {
    ...
    // Parallel block support (YAML: `parallel: true` per step)
    std::mutex results_mutex;

    for (const auto& step : config["steps"]) {
        bool parallel = step["parallel"] && step["parallel"].as<bool>();
        std::string cmd_name = step["command"].as<std::string>();
        ...
        if (!cmd) {
            ...
            if (step["rollback"]) {
                // Optionally call rollback command here.
            }
            break;
        }
        ...
        if (step["params"]) {
            ...
            // Substitute result piping placeholders
            params = resolve_placeholders(params, step_results);
        }

        auto exec_func = [&]() {
            try {
                result.output = cmd->execute(params); // Now returns output (see ICommand below)
                result.success = true;
            } catch (const std::exception& ex) {
                result.success = false;
                result.error_message = ex.what();
                // Optionally call rollback command if step["rollback"]
            }
        ...
        if (parallel) {
            std::thread(exec_func).detach();
        } else {
            exec_func();
            if (!result.success) break; // Stop on error for sequential
        }
    }
    // Wait for any parallel steps to finish (simplest approach: sleep, or use futures for strict sync)
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    return std::all_of(step_results.begin(), step_results.end(), [](const auto& r) { return r.success; });
}

web::json::value WorkflowRunner::run_and_report(const std::string& yaml_content) {
    YAML::Node config = YAML::Load(yaml_content);
    std::vector<WorkflowStepResult> results;
    // (Same as above, but reading from YAML::Node directly.)
    // ... implement similar loop, push to results ...
